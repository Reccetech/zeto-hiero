# Shielded Pool — Walkthrough Scripts

Per-transaction TypeScript files for the walkthrough in
[`docs/tutorial.md`](../../docs/tutorial.md). They live **inside the `zeto-hiero` repo**
(`zeto-hiero/examples/walkthrough/`) so every import — Hardhat, the compiled artifacts, the witness
helpers in `../../test/lib/zeto-witness.ts`, and the SDK — resolves and the scripts actually run.
...
| File | Transaction | How to run (from repo root) |
|---|---|---|
| `01-create-accounts.ts` | `AccountCreateTransaction` (Alice, Bob) | `npx ts-node examples/walkthrough/01-create-accounts.ts` |
| `02-create-token.ts` | `TokenCreateTransaction` (writes `TOKEN_ID`/`UNDERLYING_TOKEN_ADDRESS` to `.env`) | `npx ts-node examples/walkthrough/02-create-token.ts` |
| `03-associate-token.ts` | `TokenAssociateTransaction` | `npx ts-node examples/walkthrough/03-associate-token.ts` |
| `04-fund-alice.ts` | `TransferTransaction` | `npx ts-node examples/walkthrough/04-fund-alice.ts` |
| `05-deploy-pool.ts` | deploy verifiers + UUPS pool + `setupHTS` | `npx hardhat run examples/walkthrough/05-deploy-pool.ts --network hedera_testnet` |
| `06-deposit.ts` | `approve` + `deposit` (+ proof) | `npx hardhat run examples/walkthrough/06-deposit.ts --network hedera_testnet` |
| `07-transfer.ts` | private `transfer` (+ proof, event decrypt) | `npx hardhat run examples/walkthrough/07-transfer.ts --network hedera_testnet` |
| `08-withdraw.ts` | `withdraw` (+ proof) | `npx hardhat run examples/walkthrough/08-withdraw.ts --network hedera_testnet` |
| `09-reconcile.ts` | `balanceOf` + `shieldedSupply` | `npx hardhat run examples/walkthrough/09-reconcile.ts --network hedera_testnet` |
...
# regenerate from a fresh clone via ../../docs/rebuild-circuits.md
```
...
npx ts-node examples/walkthrough/01-create-accounts.ts   # paste the printed ALICE_/BOB_ lines into ./.env
npx ts-node examples/walkthrough/02-create-token.ts      # writes TOKEN_ID + UNDERLYING_TOKEN_ADDRESS
